...
if (isset($_POST['ri_form_submit']) && $_POST['ri_form_submit'] == 'yes') {
	check_admin_referer('ri_form_add');
	...
	{
		$status = rilb_cls_dbquery::rilb_action_ins($form, "insert");
		if($status == 'inserted') {
			$ri_success = __('New image details was successfully added.', 'random-image-light-box');
		}
		else {
...
?>
<div class="form-wrap">
	<h1 class="wp-heading-inline"><?php _e('Add image', 'random-image-light-box'); ?></h1><br /><br />
	<form name="ri_form" method="post" action="#" onsubmit="return _ri_submit()" >      
      ...
      <input name="ri_image" type="text" id="ri_image" value="" size="60" />
	  <input type="button" name="upload-btn" id="upload-btn" class="button-secondary" value="Upload Image">
      <p><?php _e('Where is the image located on the internet. Thumbnail image.', 'random-image-light-box'); ?> <br />(ex: http://www.gopiplus.com/work/wp-content/uploads/sample.jpg)</p>
	  
	  <label for="tag-link"><strong><?php _e('Image title', 'random-image-light-box'); ?></strong></label>
      <input name="ri_title" type="text" id="ri_title" value="" size="60" />
	  ...
	  <input name="ri_width" type="text" id="ri_width" value="0" maxlength="3" />
      ...
      <input name="ri_image_link" type="text" id="ri_image_link" value="" size="60" />
	  <input type="button" name="upload-btn1" id="upload-btn1" class="button-secondary" value="Upload Image">
      <p><?php _e('Where is the image located on the internet. Big image.', 'random-image-light-box'); ?> <br />(ex: http://www.gopiplus.com/work/wp-content/uploads/sample.jpg)</p>
		...
		<select name="ri_group" id="ri_group">
			...
			if(count($groups) > 0) {
				foreach ($groups as $group) {
					?>
					<option value="<?php echo stripslashes($group["ri_group"]); ?>">
      ...
      <input name="ri_id" id="ri_id" type="hidden" value="">
	  ...
	  <?php wp_nonce_field('ri_form_add'); ?>
    </form>
</div>
</div>